...
def is_witaqua_gerrit(remote_url):
    p = urllib.parse.urlparse(remote_url)
    return p.hostname == "gerrit.witaqua.org"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        patches from WitAqua's Gerrit instance (or any gerrit instance of your choosing)
    ...
    parser.add_argument(
        "-g",
        "--gerrit",
        default="https://gerrit.witaqua.org",
        metavar="",
...
def do_git_fetch_pull(args, item):
    ...
    # Try fetching from GitHub first if using WitAqua gerrit
    if is_witaqua_gerrit(args.gerrit):
        if args.verbose:
    ...
    # If not using the witaqua gerrit or github failed, fetch from gerrit.
    if args.verbose:
        if is_witaqua_gerrit(args.gerrit):
            print(

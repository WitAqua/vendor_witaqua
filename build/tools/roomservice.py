...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from WitAqua-Devices Github (http://github.com/WitAqua-Devices)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:WitAqua-Devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
        result = json.loads(urllib.request.urlopen(githubreq, timeout=10).read().decode())
    except urllib.error.URLError:
        print("Failed to search from GitHub")
        sys.exit(1)
    except ValueError:
...
def get_default_or_fallback_revision(repo_name):
    default_revision = get_remote_revision_from_snippet(".repo/manifests/snippets/witaqua.xml", "witaqua-devices")
...
print("Repository for %s not found in the WitAqua-Devices Github repository list. If this is an error, you may need to manually add it to your local_manifests/roomservice.xml." % device)

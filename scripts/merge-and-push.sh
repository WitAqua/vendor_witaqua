...
# --- Exclude Settings (スペース区切りでスキップしたい項目を指定) ---
# 1. リモート名で除外 (例: gitlab, github, aosp)
...
# --- Dynamically Extract Default Upstream Branch from default.xml ---
...
# --- Dynamically Extract Default Target Branch from witaqua.xml (Multi-line Safe) ---
DEFAULT_TARGET_BRANCH=$(tr '\n' ' ' < "${MANIFEST_FILE}" | sed -n 's/.*<remote[^>]*name="witaqua"[^>]*revision="\([^"]*\)".*/\1/p' | sed 's|^refs/heads/||')

# Fallback check if parsing somehow fails
if [ -z "${DEFAULT_TARGET_BRANCH}" ]; then
    echo "[!] Warning: Could not parse witaqua remote revision. Falling back to 16.2."
